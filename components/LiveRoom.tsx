    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
            ...
            if (message.serverContent?.outputTranscription) {
              ...
              if (message.serverContent?.turnComplete) {
                const newEntry: TranscriptionEntry = {
                  ...
                  text: currentTranscriptionRef.current || "Audio pattern detected",
          ...
          onerror: (e) => console.error("Neural Echo Error:", e),
          onclose: () => {
        ...
        config: {
          ...
          systemInstruction: `You are the EchoHub AI co-host.
      ...
      console.error("Nexus Link Failure:", err);
      setIsConnecting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[200] bg-[var(--bg-main)] flex flex-col overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-[var(--bg-main)]">
        <div className="max-w-3xl mx-auto space-y-10 py-6 px-6">
          ...
          <section className="bg-[var(--bg-secondary)] rounded-[32px] p-4 flex flex-col sm:flex-row items-center justify-between shadow-sm border border-[var(--glass-border)] gap-4">
             <div className="flex items-center gap-4">
                <button onClick={() => setIsLangSelectorOpen(true)} className="flex items-center gap-2 hover:bg-[var(--bg-main)] px-4 py-2 rounded-2xl">
                  <span className="text-[10px] font-black text-[var(--accent)] uppercase tracking-widest">Echo: {targetTranslationLang.toUpperCase()}</span>
                ...
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-black text-[var(--text-main)] uppercase tracking-widest">Nexus Link {isConnecting ? 'Linking...' : 'Active'}</span>
                </div>
             </div>
             <div className="flex items-center gap-2">
                <button onClick={() => setIsFeedOpen(!isFeedOpen)} className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest hover:text-[var(--accent)] flex items-center gap-2 px-4 py-2">
                  {isFeedOpen ? 'Hide Pulse' : 'Neural Pulse'}
                </button>
                {lastSessionRecord && (
                  <button onClick={() => setShowSessionReport(true)} className="bg-[var(--accent)] text-white px-5 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-widest shadow-lg">View Archive</button>
                )}
             </div>
          </section>
          ...
          <section className="pb-40">
            <h3 className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] mb-10 text-center italic opacity-40">Audience Pulse</h3>
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-y-10 justify-items-center opacity-60">
              {[...Array(12)].map((_, i) => (
                <Avatar key={i} user={{ id: `l-${i}`, name: `Listener`, avatar: `https://picsum.photos/seed/lstn${i}/100`, role: UserRole.LISTENER, isMuted: true, handRaised: false }} size="sm" />
              ))}
            </div>
          </section>
        </div>
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-[60] bg-[var(--bg-main)]/95 backdrop-blur-2xl rounded-t-[48px] shadow-[0_-30px_80px_rgba(0,0,0,0.2)] border-t border-[var(--glass-border)] transition-transform duration-700 ${isFeedOpen || isMixerOpen ? 'translate-y-0 h-[65vh]' : 'translate-y-full'}`}>
         <div className="p-8 flex flex-col h-full">
            <div className="flex justify-between items-center mb-8">
               <h4 className="text-sm font-black uppercase tracking-[0.3em] text-[var(--text-main)] italic">{isMixerOpen ? 'Stage Mixer' : 'Neural Pulse Feed'}</h4>
                    ...
                    {room.speakers.map(speaker => (
                      <div key={speaker.id} className="bg-[var(--bg-secondary)] p-6 rounded-[32px] border border-[var(--glass-border)] flex items-center justify-between gap-8 group transition-all">
                        <div className="flex items-center gap-5 shrink-0">
                          ...
                          <div><h5 className="font-black text-[var(--text-main)] uppercase italic">{speaker.name}</h5><p className="text-[9px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Active Echo</p></div>
                        </div>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[70] w-full max-w-xl px-6">
        <div className="bg-[var(--bg-main)]/90 backdrop-blur-3xl rounded-full p-4 flex justify-between items-center shadow-[0_32px_64px_rgba(0,0,0,0.4)] border border-white/10">
           ...
           <div className="flex items-center gap-4">
              ...
              <button onClick={() => setIsHandRaised(!isHandRaised)} className={`p-7 rounded-full transition-all transform hover:scale-105 shadow-2xl ${isHandRaised ? 'bg-yellow-400 text-white' : 'bg-[var(--bg-secondary)] text-slate-400'}`}><span className="text-3xl leading-none">✋</span></button>
           </div>
           <div className="flex items-center gap-2">
      ...
      {isFinalizing && (
        <div className="fixed inset-0 z-[300] bg-black/90 backdrop-blur-3xl flex flex-col items-center justify-center animate-in fade-in">
           ...
           <h2 className="text-3xl font-black uppercase tracking-[0.5em] text-white italic animate-pulse">Archiving to Vault</h2>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[400] bg-slate-950/95 backdrop-blur-3xl flex items-center justify-center p-6 animate-in zoom-in-95 duration-500">
           <div className="w-full max-w-4xl bg-[var(--bg-secondary)] rounded-[64px] border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
              <div className="p-12 bg-gradient-to-br from-indigo-700 via-accent to-indigo-900 text-white shrink-0 flex justify-between items-center">
                 <div><h2 className="text-5xl font-black uppercase tracking-tighter italic">Vault Locked</h2><p className="text-[10px] font-bold uppercase tracking-[0.8em] opacity-60">EchoHub Neural Archive</p></div>
              ...
              <div className="flex-1 overflow-y-auto p-12 custom-scrollbar bg-main/20">
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="space-y-6">
                       <h3 className="text-[14px] font-black text-indigo-500 uppercase tracking-[0.4em] flex items-center gap-3"><span className="w-8 h-0.5 bg-indigo-500" />AI Neural Insight</h3>
